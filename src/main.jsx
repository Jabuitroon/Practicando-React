...
export function TicTacToe ({ xIsNext, squares, onPlay }) { // TicTacToe es controlado por las props que recibe.
  ...
  const handleClick = index => {
    ...
    const nextSquares = squares.slice() // Almacenar un historial de movimientos
    if (xIsNext) {
      nextSquares[index] = '❌'
    } else {
      nextSquares[index] = '🧿'
  ...
  // Es un hook que recibe como primer paraetro un codigo "Arbitrario" que se ejecutará segun serenderiza el componente (Si no tiene dependencias)
  useEffect(() => {
